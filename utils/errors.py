...
def compute_errors(u, tvec, problem, q=2, mode="all"):
    ...
    if q == -1:  # Error energy
        return np.sum(err_norms, axis=0)  # sum the error over the components
    ...
    if mode == "l2":
        ...
        err_norms = integrate_1D(err_norms, tvec[1:], method='midpoint', axis=axis)
        sol_norms = integrate_1D(sol_norms, tvec[1:], method='midpoint', axis=axis)
