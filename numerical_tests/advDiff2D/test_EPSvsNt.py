...
def main():
    ...
    errors_all = {"IMEX-RB": np.empty((len(Nt_values), len(eps_values),
                                       Nt_values[-1], len(Nx_values))),
    ...
    times = {"IMEX-RB": np.zeros((len(Nt_values), len(eps_values),
             ...
             "FE": np.empty((len(Nt_values), len(Nx_values)))}
    subiters = {"IMEX-RB": np.empty((len(Nt_values), len(eps_values),
                                     Nt_values[-1], len(Nx_values))),
                "BE": None, "FE": None}
    ...
    for idx_Nx, Nx in enumerate(Nx_values):
        ...
        Ny = Nx
        # Setup problem
        problem = AdvDiff2D(Nx, Ny, Lx, Ly, mu=mu,
                            sigma=sigma, vx=vx, vy=vy, center=center)
        ...
        if idx_Nx == 0:
            ...
            test_dir = create_test_directory(os.path.join(results_dir, problem.name),
                                             testname)
        epsilon_refs[idx_Nx] = 1.0 / cond_sparse(
            problem.A,
            tol=tol_cond_NtFE,
        ...
        logger.debug(f"Considering reference epsilon = {epsilon_refs[idx_Nx]:.4e}")
        ...
        for cnt_Nt, _Nt in enumerate(Nt_values):
            ...
            # Run IMEX-RB for different epsilon values
            for cnt_eps, eps in enumerate(eps_values):
                epsilon = epsilon_refs[idx_Nx] * eps
                logger.info(f"Solving for epsilon={epsilon:.4e}")

                uIMEX, _, iters = imexrb(problem, u0, [t0, T], _Nt, epsilon, N,
                                        maxsubiter)
                if n_solves > 0:
                    ...
                    _t = timer.repeat(number=1, repeat=n_solves)
                    times["IMEX-RB"][cnt_Nt, cnt_eps, idx_Nx] += np.mean(_t)

                # Store subiterates
                subiters["IMEX-RB"][cnt_Nt, cnt_eps, :_Nt, idx_Nx] = iters

                # Compute errors
                errors_all["IMEX-RB"][cnt_Nt, cnt_eps, :_Nt, idx_Nx] = compute_errors(
                    uIMEX, tvec, problem, mode="all"
                )
                errors_l2["IMEX-RB"][cnt_Nt, cnt_eps, idx_Nx] = integrate_1D(
                    errors_all["IMEX-RB"][cnt_Nt, cnt_eps, :_Nt, idx_Nx],
                    tvec[1:]
                )
    ...
    np.savez(
        ...
        times=times,
        subiters=subiters,
        eps_values=eps_values[:, np.newaxis] * epsilon_refs,
        Nt_values=Nt_values,
        Nx_values=Nx_values
    )
